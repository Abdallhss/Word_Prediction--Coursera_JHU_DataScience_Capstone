...
clean_corpus  <- function(corpus) {
  ...
  #corpus    <- tm_map(corpus, content_transformer(tolower)) #Wrong
  corpus    <- tm_map(corpus, content_transformer(function(x) {return (gsub("-", " ", x))}))
  #corpus    <- tm_map(corpus, content_transformer(function(x) {return (gsub("'", "", x))})) #Wrong
...
word_predict <- function(input_word) {
  ...
  while (found != TRUE && wc >= 1) {
    ...
    next_word <- which(ngram_ref$initial==word_ref)
    my_NA <- is.na(next_word[1])
    if (my_NA == FALSE) {
      next_word <- ngram_ref[next_word,]
      in_stop <- which(!(next_word$last_word %in% stop))
      #print(in_stop)
      #next_word <- next_word[union(in_stop,1:5),]
      
      next_word_stop <- next_word
      next_word_stop$frequency <- as.integer(next_word_stop$frequency)
      ngram_ref$frequency <- as.integer(ngram_ref$frequency)
      next_word_stop <- next_word_stop[complete.cases(next_word_stop), ]
      num_rows  <- min(nrow(next_word_stop),10)
      next_word_stop <- mutate(next_word_stop, overall.probability = round(100*next_word_stop$frequency / sum(ngram_ref$frequency[which(ngram_ref$initial==word_ref)]),2),
                          top10.probability = round(100*next_word_stop$frequency / sum(ngram_ref$frequency[which(ngram_ref$initial==word_ref)[1:num_rows]]),2))
      found <- TRUE
      #msg <- paste("wc:", wc, "word:", word_ref, "ngram_index:",ngram_index, "found:",found)
      #print(msg)
      #######
      next_word <- next_word[in_stop,]
  ...
  rownames(next_word_stop) <- NULL
  rownames(next_word) <- NULL
  return(list("stop" = next_word_stop,"non_stop" = next_word))
}    
...
# Create 100 random samples (0.01) of all corarpora (twitter,News,Blogs),
# then summarize in groups
...
ngram_list <- list(ngram_1_df, ngram_2_df, ngram_3_df, ngram_4_df, ngram_5_df)

#next_word = word_predict("What I told")
#next_word["stop"]
#next_word["non_stop"]
